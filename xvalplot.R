...
find_lambda <- function(summary_table, n_se = c(1, 2, 3)) {
  APE    <- as.numeric(summary_table$APE)
  SEPE   <- as.numeric(summary_table$SEPE)
  lambda <- summary_table$lambda
  ...
  ape_min <- min(APE, na.rm = TRUE)
  for (k in n_se) {
    thresh <- ape_min + k * SEPE[which.min(APE)]
    
    lambda_kse <- max(lambda[APE <= thresh], na.rm = TRUE)
    result[[paste0("lambda", k, "se")]] <- lambda_kse
  }
