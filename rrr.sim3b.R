...
rrr.sim3b = function (n = 100, p = 30, q.mix = c(5, 20, 5), nrank = 2, intercept = rep(0.5, 30), mis.prop = 0.2) 
{
  ...
  # ---------------------------------------------------------------------------------------------
  for (j in 6:7){
     X[, j] = categorize(X[, j], 2)$xo - 1
  }
  
  
  for(j in 8:p){
    X[ , j] = categorize(X[, j], 4)$xo
  }
  # ---------------------------------------------------------------------------------------------
  
  u0 <- matrix(nrow = p, ncol = nrank, rnorm(p * nrank, 1, 
                                             0.2))
  umatrix = matrix(sample(c(1, -1), p * nrank, replace = TRUE), nrow = p, ncol = nrank)
  u0 = u0 * umatrix
  u0 <- qr.Q(qr(u0))
  ...
  if (sum(familygroup == 3) > 0) {
    ...
    # per variable 
    for(j in 1:Po){
      qbr <- quantile(MUo[, j], probs = seq(0, 1, length.out = 5))
      PP = plogis(outer(rep(1, n), qbr) - outer(MUo[, j], rep(1, length(qbr))))
