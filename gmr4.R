...
gmr4 = function(Yn = NULL, Yb = NULL, Yo = NULL, X, S = 2, Xscale,lambda.lasso = 0, lambda.ridge = 0, lambda.glasso = 0, trace = TRUE, maxiter = 65536, dcrit = 1e-6){
  # function for generalized mixed regularized reduced rank regression
  ...
  #'           - "N"(numeric)
  #'           - "MS" (monotonespline)
  #'           - "NMS" (nonmonotonespline)
  #'           - "O" (ordinal)
  #'           - "C" (nominal) 
  #' @param penalties vector of length 3 with penalty parameters for 
  #'            - lasso (lambda_l)
  #'            - ridge (lambda_r)
  #'            - group lasso (lmabda-g) 
  #'            - for example penalties = c(0,2,0): only ridge
  #'            - default: no penalty (0,0,0)
  ...
  # starting values
  eig.out = eigen(t(PHI) %*% PHI + 1 * diag(P))
  ...
  output = list(
    ...
    iter = iter,
    loss = loss.new,
    deviance = dev.n + dev.b + dev.o,
    rdeviance = c(rdevs.n, rdevs.b, rdevs.o),
    npar = npar,
    npar.change = npar.change, 
