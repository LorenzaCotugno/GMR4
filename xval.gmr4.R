xval.gmr4 = function(Yn, Yb, Yo, X, Xscale, S = 2, K = 5, repeats = 1, lambda.lasso.seq = 0, lambda.ridge.seq = 0, lambda.glasso.seq = 0){
      ...
      for(k in 1:K){
        ...
        for (r in 1:R) {
          ...
          if (Rb > 0 && r %in% Bseq) {
            ...
            YY = YY[idx] # select test cases
            PI = Yhat[[r]]
            prederror[r] = -(sum(log(PI[which(YY == 1)])) + sum(log(1 - PI[which(YY == -1)])))/length(YY)
          }
          if (Ro > 0 && r %in% Oseq) {
            PI = Yhat[[r]]
            YY = Ylist[[r]] # matrix
